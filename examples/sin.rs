...
fn bass(duration: Duration, frequency: f64, amp: f64) -> Value<f64> {
    ...
    let ffreq_env: Value<f64> = Linear::new(0.01, 1.0, 0.01, duration.as_secs_f64(), 0.1).into();
...
fn kick_drum(_duration: Duration, _frequency: f64, amp: f64) -> Value<f64> {
    ...
    let cenv = Linear::new(0.001, 1.0, 0.01, 0.0, 0.0);
    let oenv: Value<f64> = Linear::new(0.001, 1.0, 0.09, 0.0, 0.06).into();
    let osc = WaveTableSynth::sin(58.0 * oenv);
    let env = Linear::new(0.001, 1.0, 0.0, 0.02, 0.005);
...
fn snare_drum(_duration: Duration, _frequency: f64, amp: f64) -> Value<f64> {
    ...
    let osc = 0.5 * osc;
    let renv: Value<f64> = Linear::new(0.001, 1.0, 0.09, 0.0, 0.12).into();
