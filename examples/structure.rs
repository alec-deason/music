...
fn harmonic_structure<'a>(
    chord_durations: &[(f64, f64)],
    scale: &'a Scale,
    len: f64,
) -> HarmonicStructure<'a> {
    ...
    while current_length < len
        || (progression.is_empty() || (progression.last().unwrap().2 != terminal_chord))
    {
        let chord_duration = chord_durations
            .choose_weighted(&mut rng, |d| d.1)
            .unwrap()
            .0;
...
fn main() {
    ...
    let pad: Value<f64> = sequence_from_iterator(structure.clone().into_iter().map(
        move |(num_beats, _, chord)| {
    ...
    let bass: Value<f64> =
        sequence_from_iterator(structure.into_iter().map(move |(num_beats, _, chord)| {
            let num_beats = num_beats;
            let chord = chord;
            (
                Duration::from_millis((num_beats * beat) as u64),
                sequence_from_iterator((0..num_beats as usize).map(move |_| {
                    let note = Note {
        ...
        .into();
    let voice_1: Value<f64> = sequence_from_iterator(voice_1_melody.into_iter().map(
        move |(num_beats, tone, amp)| {
            let note = Note {
    ...
    .into();
    let voice_2: Value<f64> = sequence_from_iterator(voice_2_melody.into_iter().map(
        move |(num_beats, tone, amp)| {
            let note = Note {
                duration: Duration::from_millis((num_beats * beat) as u64),
                frequency: (tone).frequency_from_midi() as f64,
