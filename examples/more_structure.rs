...
use music::{
    effect::*, envelope::*, filter::*, note::*, oscillator::sampler::*, oscillator::string::*,
    oscillator::*, sequence::*, value::*, Env,
};
...
mod plan {
    ...
    fn smooth_progression(
        ...
        let mut state = chord_map
            .iter()
            .enumerate()
            .filter(|(_, s)| s.0 == current as i32)
            .nth(0)
            .unwrap_or((4, &(6, vec![2, 7])))
            .0;
    ...
    pub fn new(rng: &mut impl Rng) -> Parts {
        ...
        for _ in 0..10 {
            if parts.len() > 0 && rng.gen::<f64>() < 0.6 {
                ...
                let progression =
                    smooth_progression(&key, &last_end, 8, augmentation_prob, density, rng);
                ...
                match t {
                        ...
                        notes.push((
                            *dur,
                            Some(
                                chord
                                    .iter()
                                    .map(|t| ((*t as i32 + octave * 12) as u32, amp))
                                    .collect(),
                            ),
                        ));
                        beat_clock += dur;
                    ...
                    HarmonyType::ArpeggiatedChord(direction) => {
                        for i in 0..*dur as usize {
                            ...
                            notes.push((
                                1.0,
                                Some(vec![(
                                    (chord[i % chord.len()] as i32 + octave * 12) as u32,
                                    amp,
                                )]),
                            ));
                            beat_clock += 1.0;
                    ...
                    HarmonyType::RepeatedRoot => {
                        for _ in 0..*dur as u32 {
                            ...
                            notes.push((
                                1.0,
                                Some(vec![((chord[0] as i32 + octave * 12) as u32, amp)]),
                            ));
                            beat_clock += 1.0;
                        }
                    }
            ...
            for (dur, _inten, local_density, chord) in progression {
                ...
                while beats_remaining > 0.0 {
                    ...
                    if rng.gen::<f64>() < density {
                        if beats_remaining < 3.0 / local_subdivision
                            || rng.gen::<f64>() > passing_note_prob
                        {
                            let (accented, amp) = accent(beat_clock as u32);
                            let tone = if rng.gen::<f64>() < break_direction_prob {
                                last_chord_position = (last_chord_position + direction)
                                    .min(chord.len() as i32 - 1)
                                    .max(0);
                            ...
                            let (accented, amp) = accent(beat_clock as u32);
                            notes.push((
                                1.0 / local_subdivision,
                                Some(vec![(Tone::ChordTone(tone as usize), amp)]),
                            ));
                            beat_clock += 1.0 / local_subdivision;
                            beats_remaining -= 1.0 / local_subdivision;
                            ...
                            notes.push((
                                1.0 / local_subdivision,
                                Some(vec![(
                                    Tone::StepFromChordTone(tone as usize, direction),
                                    amp,
                                )]),
                            ));
                            beat_clock += 1.0 / local_subdivision;
                            ...
                            notes.push((
                                1.0 / local_subdivision,
                                Some(vec![(
                                    Tone::AdjacentChordTone(tone as usize, direction),
                                    amp,
                                )]),
                            ));
                            beat_clock += 1.0 / local_subdivision;
                            ...
                            for (tone, amp) in tone_templates {
                                let tone = match tone {
                                    ...
                                    Tone::StepFromChordTone(idx, direction) => {
                                        let mut tone =
                                            chord[idx.min(chord.len() - 1)] as i32 + direction;
                                        while key.degree(tone as u32).is_none() {
                                            tone += direction;
                                        }
                                        tone
                                    }
                                    Tone::AdjacentChordTone(idx, direction) => {
                                        ...
                                        if direction < 0 || idx < chord.len() - 1 {
                                            while !chord.contains(&(tone as u32)) {
                                                tone += direction;
                                            }
                                        }
                                        tone
...
fn pad<'a>(note: Note) -> Value<'a, f64> {
    ...
    let freq2: Value<f64> = (note.frequency.midi_from_frequency() + 0.05)
        .frequency_from_midi()
        .into();
...
fn chorus<'a>(note: Note) -> Value<'a, f64> {
    ...
    let env: Value<f64> = ADSR::new()
        .attack(0.1)
        .sustain(1.0)
        .duration(note.duration.as_secs_f64() - 0.1)
        .release(0.1)
        .curve(1.0)
        .into();
...
pub fn main() {
    ...
    let voices = voicing::new(
        &plan::new(&mut rng),
        &[
            voicing::Voice::Melody(0, 0.9),
            if rng.gen::<f64>() > 0.5 {
                voicing::Voice::Harmony(
                    voicing::HarmonyType::ArpeggiatedChord(rng.gen_range(0, 3)),
                    -2,
                )
            } else {
    ...
    let banjo = SampleSet::from_directory(
        &"samples/banjo",
        &Regex::new(r".*/banjo_(?P<note>[A-G]s?)(?P<octave>[0-9])_very-long_forte_normal.mp3")
            .unwrap(),
    );
    let play_banjo = |note: &Note| banjo.play(note.frequency).unwrap() * note.amplitude;

    let swing = 0.0;
    let mut beat_clock = 0.0;
    let melody_string = rng.gen_range(0, 3);
    let buzzyness = rng.gen_range(0.05, 2.0);
    let melody_voice: Value<f64> =
        sequence_from_iterator((&voices[0]).into_iter().map(move |(num_beats, note)| {
            if let Some(notes) = note {
                ...
                for (tone, amp) in notes {
                    ...
                    if melody_string == 0 {
                        let mut pluck: Value<f64> =
                            PluckedString::new(note.frequency / 2.0, 0.6).into();
    ...
    let bass: Value<f64> =
        sequence_from_iterator((&voices[1]).into_iter().map(move |(num_beats, note)| {
            if let Some(notes) = note {
                let mut sig: Value<f64> = 0.0.into();
                let beat = if beat_clock % 0.5 == 0.0 {
                    beat * (1.0 + swing)
    ...
    let pads: Value<f64> =
        sequence_from_iterator((&voices[2]).into_iter().map(move |(num_beats, note)| {
            if let Some(notes) = note {
                let mut sig: Value<f64> = 0.0.into();
                for (tone, amp) in notes {
                    let note = Note {
