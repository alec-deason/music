...
use music::{
    composition::*, effect::*, filter::*, note::*, oscillator::sampler::*, oscillator::string::*,
    sequence::*, value::*, Env,
};
...
fn consonance(a: u32, b: u32) -> bool {
    ...
    let acceptable_ratios: Vec<f64> = (1..small_number_limit)
        .map(|a| {
            (1..small_number_limit)
                .map(|b| a as f64 / b as f64)
                .collect::<Vec<f64>>()
        })
        .flatten()
        .collect();
...
fn fill_chords(
    composition: &mut Composition<Message>,
    key: &Scale,
    beats: u32,
    rng: &mut impl Rng,
) {
    ...
    .map(|(s, ns)| {
        (
            *s,
            ns.iter()
                .map(|n| (*n, rng.gen_range(0.0001, 1.0)))
...
fn fill_from_chords(
    ...
    let tone_map: Vec<(Option<i32>, Vec<usize>)> = (0..10)
        .map(|i| {
            (
                if rng.gen::<f64>() < density {
                    Some(rng.gen_range(-5, 6))
                } else {
                    None
                },
            ...
            if let Some(jump) = jump {
                ...
                if rng.gen::<f64>() < decoration_prob * decoration_prob_jiggle {
                    if previous.is_some() && rng.gen::<f64>() < passing_note_prob {
                        ...
                        while key.degree(passing as u32).is_none() {
                            passing += direction;
                        }
...
fn fill_from_accompanyment(
    ...
    while len <= beats as f64 {
        ...
        let mut key = None;
        let mut notes = vec![];
        let beat = pattern[i % pattern.len()] as f64
            * (1.0
                / if rng.gen::<f64>() < double_prob {
                    8.0
                } else {
                    4.0
                });
...
fn bass_fill(
    ...
    while len <= beats as f64 {
        ...
        if mode {
            ...
            let idx = if note % 2 == 0 { 0 } else { chord.len() - 1 };
...
fn render_voice<'a>(
    voice: &Voice,
    instrument: &'a dyn Fn(&Note) -> Value<'a, f64>,
    beat: f64,
) -> Value<'a, f64> {
    sequence_from_iterator(voice.clone().into_iter().map(move |(num_beats, chord)| {
...
pub fn main() {
    ...
    let banjo = SampleSet::from_directory(
        &"samples/banjo",
        &Regex::new(
            r".*/banjo_(?P<note>[A-G]s?)(?P<octave>[0-9])_very-long_forte_normal_truncated.mp3",
        )
        .unwrap(),
    );
    let play_banjo = |note: &Note| banjo.play(note.frequency).unwrap() * note.amplitude;
    let play_bass_banjo = |note: &Note| banjo.play(note.frequency / 2.0).unwrap() * note.amplitude;
    let trumpet = SampleSet::from_directory(
        &"samples/trumpet",
        &Regex::new(
            r".*/trumpet_(?P<note>[A-G]s?)(?P<octave>[0-9])_025_pianissimo_normal_truncated.mp3",
        )
        .unwrap(),
    );
    let play_trumpet = |note: &Note| trumpet.play(note.frequency).unwrap() * note.amplitude;
    ...
    /*
    sig = sig + hass_shift(
           render_voice(composition.voice(accompanyment).unwrap(), &play_bass_banjo, beat) * 0.6,
           -3.0/1000.0,
    );
    */
    sig = sig
    ...
    for _ in 0..total_samples / chunk_size {
        ...
        for sample in buffer_left {
            io::stdout()
                .write_f32::<LittleEndian>(sample.0 as f32 * amp)
                .unwrap();
            io::stdout()
                .write_f32::<LittleEndian>(sample.1 as f32 * amp)
                .unwrap();
        }
    }
}
